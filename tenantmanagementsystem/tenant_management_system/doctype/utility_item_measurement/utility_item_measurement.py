...
class UtilityItemMeasurement(Document):
    ...
    def get_utility_item(self):
        return frappe.get_doc("Utility Item", self.get("utility_item"))

    def get_previous_measurement(self):
        prev = frappe.db.sql(
            "SELECT name FROM `tabUtility Item Measurement` WHERE "
            "measurement_status != 'Cancelled' AND tenancy_contract = %(tc)s "
            "AND utility_item = %(ui)s "
            "AND NAME != %(me)s AND reading_date < %(rd)s "
            "ORDER BY reading_date DESC LIMIT 0, 1",
            {
                "tc": self.tenancy_contract,
                "ui": self.utility_item,
                "me": self.name,
        ...
        return frappe.get_doc("Utility Item Measurement", prev[0][0])

    def validate_reading(self):
        utility_item = self.get_utility_item()
        if (
            utility_item.measurement_type == "Meter Reading"
            and not self.is_opening_entry
        ):
            prev = self.get_previous_measurement()
    ...
    def validate_dates(self):
        if self.measurement_status == "New" and not self.is_opening_entry:
            count = frappe.db.sql(
                "SELECT count(*) FROM `tabUtility Item Measurement` WHERE "
                "measurement_status != 'Cancelled' AND tenancy_contract = %(tc)s "
                "AND utility_item = %(ui)s "
                "AND NAME != %(me)s AND reading_date >= %(rd)s ",
                {
    ...
    def check_if_older_exists(self):
        if self.measurement_status == "New" and not self.is_opening_entry:
            count = frappe.db.sql(
                "SELECT count(*) FROM `tabUtility Item Measurement` WHERE "
                "measurement_status = 'New' AND tenancy_contract = %(tc)s "
                "AND utility_item = %(ui)s AND is_opening_entry = 0 "
                "AND NAME != %(me)s",
                {"tc": self.tenancy_contract, "ui": self.utility_item, "me": self.name},
            )
            if count[0][0]:
                frappe.throw(
                    _(
    ...
    def check_opening_item(self):
        utility_item = self.get_utility_item()
        if (
            self.measurement_status == "New"
            and utility_item.measurement_type == "Meter Reading"
        ):
            count = frappe.db.sql(
                "SELECT count(*) FROM `tabUtility Item Measurement` WHERE "
                "measurement_status != 'Cancelled' AND tenancy_contract = %(tc)s "
                "AND utility_item = %(ui)s AND is_opening_entry = 1 "
                "AND NAME != %(me)s",
                {"tc": self.tenancy_contract, "ui": self.utility_item, "me": self.name},
            )
            if not count[0][0] and not self.is_opening_entry:
                frappe.throw(
                    _(
                        "For an item of measurement type of 'Meter Reading', the first entry must be an opening item."
                    )
                )
