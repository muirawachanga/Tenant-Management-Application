...
def sales_invoice_arrears(doc, event):
    ...
    arrears = frappe.db.sql(
        ...
        (doc.customer, today(), doc.tenancy_contract),
    )[0][0]
    if arrears is None or arrears <= 0:
        doc.arrears_note = ""
        return
    doc.arrears_note = "You have {0} in pending arrears, your total amount to pay is: {1}".format(
        fmt_money(arrears, 2, doc.currency),
        fmt_money(flt(doc.outstanding_amount + arrears), 2, doc.currency),
    )


def reset_utility_item_measurement_billing(doc, event):
    ut_measurement = frappe.get_list(
        "Utility Item Measurement",
        fields=["*"],
        filters=[["billing_invoice", "=", doc.name]],
    )
